...
# role name -> (build_report_spec(period), build_registry(period))
...
def list_roles() -> None:
    ...
    for role in roles:
        datasets = data_mod.list_datasets(role)
        wired = "wired (build-role)" if role in _ROLE_BUILDERS else "data only, not wired"
...
@click.option("--role", required=True, type=click.Choice(sorted(_ROLE_BUILDERS)))
@click.option("--format", "fmt", required=True, type=click.Choice(["deck", "doc"]))
...
def build_role(role: str, fmt: str, period: str | None, out_path: str | None) -> None:
    """Build a deck or doc for ROLE at PERIOD (default: the role's default period)."""
